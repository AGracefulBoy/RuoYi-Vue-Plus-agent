package org.dromara.common.llm.model.platform.doubao.chat;


import org.dromara.common.llm.model.platform.IChatService;
import org.dromara.common.llm.model.platform.doubao.converter.DouBaoChatResponseConverter;
...
@Service
public class DouBaoChatService implements IChatService {
    @Override
    public Flux<IChatResponse> stream(IChatRequest iChatRequest) {
        ...
        return iChatRequest.getStream() ?
            built.stream(new Prompt(list)).map(DouBaoChatResponseConverter::convert) :
            Flux.just(DouBaoChatResponseConverter.convert(built.call(new Prompt(list))));
